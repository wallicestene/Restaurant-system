...
const Footer = () => {
  ...
  return (
    <footer className={`fixed bottom-0 left-0 z-10 w-full px-2 bg-white text-ebony-50 font-mulish border rounded-t-md transition-all duration-300 ease-linear h-10 ${showFooter && " h-[250px] "}`}>
      <div className=" flex items-center justify-between flex-wrap h-10">
        ...
        <div>
          <button onClick={() => setShowfooter(!showFooter)} className=" h-8 w-8 grid place-items-center border rounded-full">
            <KeyboardArrowDown />
          </button>
        </div>
      </div>
      {showFooter && (
        <div className="py-5 px-1 lg:px-8  font-thin lg:grid lg:grid-cols-4 grid-cols-2 lg:gap-5 gap-8 place-items-center flex flex-row flex-wrap">
          <div>
